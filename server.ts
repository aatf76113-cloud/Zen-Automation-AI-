...
import { WorkflowEngine, generateGeminiContentSafe } from './server/workflowEngine';
import { INITIAL_TEMPLATES, INITIAL_INTEGRATIONS } from './src/data/mockData';
import { GoogleGenAI } from '@google/genai';
import { integrationsRouter } from './server/api/integrationsRouter';
import { webhooksRouter } from './server/api/webhooksRouter';
import { aiRouter } from './server/api/aiRouter';
import { whatsappRouter } from './server/api/whatsappRouter';
...
async function startServer() {
  ...
  app.all('/api/webhooks/:workflowId', async (req, res) => {
    ...
    // Find which tenant owns this workflow
    let targetWf: any = null;
    let targetOrgId: string = 'org_zain_hq';
    ...
    if (!targetWf) {
      return res.status(404).json({
        error: 'Webhook target workflow not found',
        workflowId
      });
    }
    ...
    // 4. Store Inbound Payload in Database
    ...
    try {
      ...
      // Check if all nodes in target workflow were actually executed and succeeded
      const allExecuted = execution.traces.length === targetWf.nodes.length;
      const allSucceeded = allExecuted && execution.traces.every((t) => t.status === 'success');
      ...
      if (!allSucceeded) {
        ...
        return res.status(422).json({
          received: true,
          success: false,
          status: notConnectedCount > 0 ? 'not_connected' : 'failed',
          httpStatus: 422,
          workflowId,
          executionId: execution.id,
          inboundEventId: savedEvent.id,
          error: execution.errorMessage || `اكتمل التنفيذ جزئياً: تم فحص ${execution.traces.length}/${targetWf.nodes.length} عقد. ${notConnectedCount} عقد غير متصلة بالاعتمادات.`,
      ...
      // ONLY return Status 200 OK when ALL required nodes have executed with real success
      res.status(200).json({
        received: true,
        success: true,
        status: 'success',
        ...
        stats: {
          totalNodes: targetWf.nodes.length,
          executedNodes: execution.traces.length,
          successfulNodes: execution.traces.length
        }
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
